...
def draw_label_info(frame, x_min, y_min, label, confidence, color):
  ...
  cv2.putText(frame, text,
              ...
              fontColor,
              lineType)
# NMS(Non Maximun Supplession) 의 약식 버전 
def find_max_confidence_bounding_box(bounding_box_info_list): # 7x7x2  바운딩박스 전체 갯수
  bounding_box_info_list_sorted = sorted(bounding_box_info_list,
                                                   key=itemgetter('confidence'),
                                                   reverse=True)
  max_confidence_bounding_box = bounding_box_info_list_sorted[0] # confidence 가장 높은 값 채택 

  return max_confidence_bounding_box

# bounding box dict 정의 
...
def iou(yolo_pred_boxes, ground_truth_boxes):
  ...
  square1 = (boxes1[:, :, :, 2] - boxes1[:, :, :, 0]) * (boxes1[:, :, :, 3] - boxes1[:, :, :, 1]) # 바운딩박스 1번 넓이 
  square2 = (boxes2[2] - boxes2[0]) * (boxes2[3] - boxes2[1]) # 바운딩박스 2번 넓이  

  return inter_square / (square1 + square2 - inter_square + 1e-6) # 교집합/합집합, 0~1 값 

# 클래스 별로 다른 색상으로 바운딩박스 표기
