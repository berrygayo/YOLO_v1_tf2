...
def process_each_ground_truth(original_image,
  ...
  Args:
    ...
    bbox : (max_object_num_in_batch, 4) = (ymin / height, xmin / width, ymax / height, xmax / width) # 상대좌표를 절대좌표로 변경해주기 위해 필요
  ...
  Returns:
    ...
    labels: 2-D list [object_num, 5] (xcenter (Absolute Coordinate), ycenter (Absolute Coordinate), w (Absolute Coordinate), h (Absolute Coordinate), class_num) # 절대좌표로 나옴 
  ...
  object_num = np.count_nonzero(bbox, axis=0)[0] # 실제 이미지에 object가 있는 부분을 parsing해서 가져옴
  labels = [[0, 0, 0, 0, 0]] * object_num # object 갯수만큼 리스트 생성, 0 :  리셋값 
  for i in range(object_num):
    xmin = bbox[i][1] * original_w # 절대좌표 값으로 만듬 
    ...
    xcenter = (xmin + xmax) * 1.0 / 2 * width_rate # 바운딩박스의 center 값을 구하고, 절대좌표로 구하기 위해 rate 곱 
